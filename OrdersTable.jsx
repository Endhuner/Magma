...
export default function OrdersTable({ orders, loading, db, onSaved, customers, items }) {
  ...
  const exportCmr = async () => {
    try {
      ...
      const { wb, exportId } = await buildCmrWorkbook(selectedOrders, consignee)
          ...
          <div className="formgrid grid">
            ...
            <div className="field col-12 md:col-6">
              ...
              <Calendar
                ...
                className="w-full"
              />
              {editSubmitted && !String(editOrder.requiredDate ?? '').trim() && <small className="p-error">Required date kötelező.</small>}
            </div>
          </div>
        )}
      </Dialog>
